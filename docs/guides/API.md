...
`POST /v2/app-store/notifications` is the public App Store Server Notifications V2 receiver. It accepts only Apple's bounded `{ "signedPayload": "..." }` body, verifies the outer notification and any relevant inner transaction before mutation, and returns an empty no-store response. Verified refund and revoke notifications remove the active binding; a verified refund reversal reinstates the last still-existing Inbox binding. Unrelated verified notification types are deduplicated and ignored. The relay retains only bounded notification metadata for replay control, never the signed payload or Apple transaction/account identifiers. The scheduled reconciliation path consumes Apple's bounded notification history through this same verification and idempotent persistence path, then queries Transaction History V2 with each transient affected identifier to confirm the latest non-consumable state. A durable timestamp-only checkpoint advances per Apple environment only after the complete run succeeds.

`POST /v2/operator/entitlements` is an operator-only control plane, disabled unless its dedicated secret is configured. It accepts only authenticated, idempotent, bounded `suspend`, `resume`, and `sandbox_reset` actions. Suspension changes hosted-service policy without rewriting Apple ownership. Sandbox reset is rejected for Production and removes only the exact test binding and restore target. Responses and telemetry never echo the target Inbox or operator credential.
