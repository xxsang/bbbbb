...
3. For a new HTTP-first agent setup, follow [bbbbb.app/setup](https://bbbbb.app/setup/). The setup instruction authorizes the audited temporary bootstrap and one generic Source without asking the user to choose a transport, installer, storage mode, skill directory, or Source label. The bootstrap installs or updates this skill, shows one temporary QR with its six-digit fallback, stores the approved Source privately, sends a setup test, and does not permanently install the CLI.
